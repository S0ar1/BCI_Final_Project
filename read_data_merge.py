...
def read_single_data(batch_size, index):
    ...
    for i in range(5):             #valid的批量读取和合并
        if i < 9:
            # data_path_train = './Project_Data/Train/sample0' + str(i + 1) + '.mat'
            data_path_valid = './Project_Data/Valid/sample0' + str(index + 1) + '.mat'
        else:
        ...
        if i == 0:
            ...
            validX_merge = np.vstack((validX_merge, validX))
            validY_merge = np.concatenate((validY_merge, validY), axis=0)
    ...
    dataset_valid = DataAdapter(validX_merge, validY_merge)  # 构造数据集
